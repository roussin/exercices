 ...
 * @return string
**/
function wordGen( array $db, int $index ) : string {
    $word = '';
    ...
    $word .= isset( $db[$index] ) ? $db[$index]['letter'] : '<span style="background-color:red;color:white;display:block;padding:4px 7px;">Vous devez saisir un entier compris entre 1 et ' . count( $db ) . ' (0 et ' . ( count( $db ) - 1 ) . ' pour les développeurs) !</span>'; // Si l'index existe dans le tableau, on ajoute la lettre stockée (qui sera la dernière lettre du mot généré)
...
$datas = array(
    ...
    array('letter' => 'a', 'goto' => 9)
        ...
        <p><em>Le but est d'écrire une fonction acceptant en paramètre un index de départ, et qui permet d'obtenir le mot composé à partir de l'élément à cet index et de tous les éléments suivants.<br>Par exemple l'entrée 2 devrait permettre d'obtenir "example", l'entrée 4 "probable", l'entrée 0 "able".</em></p>
        ...
        if( isset( $_POST['index'] ) ) : // Si on soumet une valeur via le formulaire,
            if( ctype_digit( $_POST['index'] ) ) : // Si cette valeur est numérique de type entier,
                if( isset( $_POST['user-type'] ) && ctype_digit( $_POST['user-type'] ) && $_POST['user-type']==1 ) // Si on soumet un type d'utilisateur,
                ...
                echo '<span style="border:black thin solid;display:block;margin:10px 0;padding:4px 7px;"><span style="background-color:red;color:white;display:block;padding:4px 7px;">Vous devez saisir une valeur numérique entière !</span></span>';
            endif;
        endif;
        ?>
        <pre><code class="php">
...
&lt;form action="" method="POST" name="frm-generator" novalidate="novalidate"&gt;
    &lt;label for="txt-index"&gt;Index de départ (entre 1 et &lt;?php echo count( $datas ); ?&gt;) :&lt;/label&gt;
    &lt;br&gt;&lt;input id="txt-index" name="index" type="number" min="1" max="&lt;?php echo count( $datas ); ?&gt;" value="&lt;?php if( isset( $_POST['index'] ) ) echo $_POST['index']; ?&gt;"&gt;
    &lt;br&gt;&lt;input&lt;?php if( !isset( $_POST['user-type'] ) || ( isset( $_POST['user-type'] ) && $_POST['user-type']==1 ) ) echo ' checked="checked"'; ?&gt; id="radio-humain" name="user-type" type="radio" value="1"&gt;&lt;label for="radio-humain"&gt;Je suis un simple humain&lt;/label&gt; / &lt;input&lt;?php if( isset( $_POST['user-type'] ) && $_POST['user-type']==0 ) echo ' checked="checked"'; ?&gt; id="radio-developer" name="user-type" type="radio" value="0"&gt;&lt;label for="radio-developer"&gt;Je suis un développeur&lt;/label&gt;
    &lt;br&gt;&lt;input type="submit" value="Générer un mot à partir de l'index"&gt;
&lt;/form&gt;
&lt;?php
if( isset( $_POST['index'] ) ) : // Si on soumet une valeur via le formulaire,
    if( ctype_digit( $_POST['index'] ) ) : // Si cette valeur est numérique de type entier,
        if( isset( $_POST['user-type'] ) && ctype_digit( $_POST['user-type'] ) ) // Si on soumet un type d'utilisateur,
            $_POST['index'] -= $_POST['user-type']; // On soustrait 1 à l'index de déaprt puisqu'un tableau débute à la clé 0 et que notre demande était un nombre entre 1 et ...
        echo '&lt;span style="border:black thin solid;display:block;margin:10px 0;padding:4px 7px;"&gt;' . wordGen( $datas, $_POST['index'] ) . '&lt;/span&gt;';
    else :
        echo '&lt;span style="border:black thin solid;display:block;margin:10px 0;padding:4px 7px;"&gt;&lt;span style="background-color:red;color:white;display:block;padding:4px 7px;"&gt;Vous devez saisir une valeur numérique entière !&lt;/span&gt;&lt;/span&gt;';
